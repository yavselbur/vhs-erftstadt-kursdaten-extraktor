...
1. Zaman durumu hesaplama: her kursun "henüz başlamadı / devam ediyor / bitti"
   ...
   (modelin kendi kendine tarih hesaplamasına güvenmiyoruz).
...
def _durumu_hesapla(ilk_gun, son_gun):
    ...
    Bu hesaplama YAPAY ZEKA MODELİNE BIRAKILMIYOR - Python ile kesin yapılıyor.
    """
    bugun = date.today()
...
# ---------------------------------------------------------------------
# 2) KATEGORİ ANAHTAR KELİMELERİ (basit kural tabanlı eşleştirme)
# ---------------------------------------------------------------------
...
# Seviye kodlarını yakalamak için (A1, A1.1, A2, B1, B1.2, B2, C1, C2 gibi)
import re as _re
_SEVIYE_DESENI = _re.compile(r"\b([ABC][12](?:\.[12])?)\b", _re.IGNORECASE)

# "Henüz başlamamış" anlamına gelen kelimeler
_BASLAMAMIS_KELIMELERI = [
    "başlayacak", "yaklaşan", "yeni başla", "anstehend", "bevorstehend",
    "upcoming", "henüz başlamayan", "gelecek",
]

# "Şu an devam eden" anlamına gelen kelimeler
...
def _durum_filtresi_bul(soru):
    """Sorudan 'sadece başlamamış' ya da 'sadece devam eden' isteği var mı anlar."""
    soru_kucuk = soru.lower()
    if any(k in soru_kucuk for k in _BASLAMAMIS_KELIMELERI):
        return "baslamamis"
    if any(k in soru_kucuk for k in _DEVAM_EDEN_KELIMELERI):
        return "devam_eden"
    return None


def _seviye_filtresi_bul(soru):
    """Sorudan bir seviye kodu (A1.1, B1 gibi) var mı bulur."""
    eslesme = _SEVIYE_DESENI.search(soru)
    return eslesme.group(1).upper() if eslesme else None


def _kategori_listesi_mi(soru):
    """
    Sorunun 'bir kategorideki TÜM kursları listele' anlamına gelip gelmediğini
    basit anahtar kelime eşleştirmesiyle tahmin eder.
    Eşleşirse (kategori_adi) döner, eşleşmezse None döner.
    """
    soru_kucuk = soru.lower()
...
def _kategori_tam_listesi(kategori_adi):
    """Bir kategorideki TÜM kursları veritabanından (embedding aramasına gerek kalmadan) çeker."""
...
# ---------------------------------------------------------------------
# NORMAL (embedding tabanlı) ARAMA + CEVAP ÜRETME
# ---------------------------------------------------------------------
...
def cevap_uret(soru):
    """
    Dışarıdan çağrılacak ana fonksiyon. İki yoldan biriyle çalışır:
    1. Soru "kategorideki tüm kursları listele" anlamına geliyorsa -> doğrudan liste (hızlı, model kullanılmaz)
